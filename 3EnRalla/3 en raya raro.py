        ...
        for event in p.event.get():
            if event.type == p.QUIT:
                ...
                if mouse_in_range and playing:
                    ...
                    board, moves, x_to_move, playing, win = reset(screen)
...
def reset(screen):
    ...
    playing = True
    win = False
    return board, moves, x_to_move, playing, win
...
def draw_board(screen):
    ...
    for i in range(DIMENSION):
        for j in range(DIMENSION):
            ...
            p.draw.rect(screen, color2, p.Rect(j*SS+50, i*SS+150, SS, SS), 5)
...
def draw_button(screen):
    ...
    text = my_font.render("RESET", 1, (255, 255, 255))
